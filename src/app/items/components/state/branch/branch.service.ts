...
export class BranchService {
  ...
  constructor(private branchStore: BranchStore,
  ...
  get(): Observable<any> {
    ...
    return this.http.get<WarehouseModel[]>(url)
      .pipe(
        tap((result: any) => {
          if (result.success) {
            this.branchStore.set(result.data);
          } else {
  ...
  add(data): Observable<any> {
    ...
    return this.http.post(url, data)
      .pipe(
        tap((result: any) => {
          if (result.success) {
            this.branchStore.add(result.data);
          } else {
            this.notificationsService.error('Error occurred', result.error);
          }
        }, error => {
  ...
  update(id, data): Observable<any> {
    ...
    return this.http.put(url, data)
      .pipe(
        tap((result: any) => {
          if (result.success) {
            this.branchStore.update(result.data.id, result.data);
          } else {
            this.notificationsService.error('Error occurred', result.error);
          }
        }, error => {
  ...
  delete(id): Observable<any> {
    ...
    return this.http.delete(url)
      .pipe(
        tap((result: any) => {
          if (result.success) {
            this.branchStore.remove(result.data.id);
          } else {
            this.notificationsService.error('Error occurred', result.error);
          }
        }, error => {
